{-# LANGUAGE BlockArguments     #-}
{-# LANGUAGE FlexibleInstances  #-}
{-# LANGUAGE LambdaCase         #-}
{-# LANGUAGE StandaloneDeriving #-}

module Test.Expr
  ( evalExprInt
  , genFullExpr
  , genFullExprZero
  , stupidEval
  ) where

import Control.Monad (replicateM)
import Data.Bool (bool)
import Data.List (intercalate, sort)
import HW2.T1 (Annotated (..), Except (..))
import HW2.T4 (Expr (..), Prim (..))
import HW2.T5 (EvaluationError (..))
import Hedgehog (Gen)
import qualified Hedgehog.Gen as Gen
import qualified Hedgehog.Range as Range
import Numeric (showFFloat)

deriving instance (Eq a) => Eq (Prim a)
deriving instance Eq Expr

deriving instance Ord (Prim Double)

instance Eq (Annotated [Prim Double] Double) where
  (==) (a :# aPrim) (b :# bPrim) = a == b && sort aPrim == sort bPrim

deriving instance (Show a, Show e) => Show (Annotated e a)
deriving instance (Eq a, Eq e) => Eq (Except e a)
deriving instance (Show a, Show e) => Show (Except e a)

deriving instance Show EvaluationError
deriving instance Eq EvaluationError

instance Show Expr where
  show (Val x) = show x
  show (Op e)  = show e

instance Show a => Show (Prim a) where
  show (Add x y) = "(" ++ show x ++ " + " ++ show y ++ ")"
  show (Sub x y) = "(" ++ show x ++ " - " ++ show y ++ ")"
  show (Mul x y) = "(" ++ show x ++ " * " ++ show y ++ ")"
  show (Div x y) = "(" ++ show x ++ " / " ++ show y ++ ")"
  show (Abs x)   = "abs(" ++ show x ++ ")"
  show (Sgn x)   = "signum(" ++ show x ++ ")"

genValInt :: Gen Expr
genValInt = Val . fromIntegral <$> Gen.int (Range.linear 1 10)

genVal :: Gen Expr
genVal = Val . read . flip (showFFloat (Just 5)) "" <$> Gen.double (Range.linearFrac 0 100)

type OpCtr = Expr -> Expr -> Prim Expr
type UnaryOpCtr = Expr -> Prim Expr

genExprBuilder' :: Int -> Bool -> [OpCtr] -> [UnaryOpCtr] -> Gen Expr
genExprBuilder' 0 _ _ _ = genVal
genExprBuilder' depth valInMiddle ops unaryOps = Gen.choice $
  (if valInMiddle then (genVal :) else id) $ map genOps ops ++ map genUnaryOps unaryOps
  where
    nextExpr :: Gen Expr
    nextExpr = genExprBuilder' (depth - 1) valInMiddle ops unaryOps

    genOps :: OpCtr -> Gen Expr
    genOps op = fmap Op $ op <$> nextExpr <*> nextExpr

    genUnaryOps :: UnaryOpCtr -> Gen Expr
    genUnaryOps op = fmap Op $ op <$> nextExpr

genExprBuilder :: Gen Int -> Bool -> [OpCtr] -> [UnaryOpCtr] -> Gen Expr
genExprBuilder genDepth valInMiddle ops unaryOps = do
  depth <- genDepth
  genExprBuilder' depth valInMiddle ops unaryOps

genExprNonZero :: [OpCtr] -> [UnaryOpCtr] -> Gen Expr
genExprNonZero = genExprBuilder (Gen.int $ Range.linear 1 5) True

genExprZero :: [OpCtr] -> [UnaryOpCtr] -> Gen Expr
genExprZero = genExprBuilder (Gen.int $ Range.linear 1 5) False

genFullExpr :: Gen Expr
genFullExpr = genExprNonZero [Add, Mul, Sub, Div] [Abs, Sgn]

genFullExprZero :: Gen Expr
genFullExprZero = genExprZero [Add, Mul, Sub, Div] [Abs, Sgn]

evalExprInt' :: (Double -> Double -> Double) -> Expr -> Expr -> Double
evalExprInt' f a b = f (evalExprInt a) (evalExprInt b)

evalExprInt'' :: (Double -> Double) -> Expr -> Double
evalExprInt'' f a = f (evalExprInt a)

evalExprInt :: Expr -> Double
evalExprInt (Val x)        = x
evalExprInt (Op (Add a b)) = evalExprInt' (+) a b
evalExprInt (Op (Sub a b)) = evalExprInt' (-) a b
evalExprInt (Op (Div a b)) = evalExprInt' (/) a b
evalExprInt (Op (Mul a b)) = evalExprInt' (*) a b
evalExprInt (Op (Abs a))   = evalExprInt'' abs a
evalExprInt (Op (Sgn a))   = evalExprInt'' signum a

isSuccess :: Except e a -> Bool
isSuccess (Success _) = True
isSuccess _           = False

fromSuccess :: Except e a -> a
fromSuccess (Success a) = a
fromSuccess _           = undefined

fromError :: Except e a -> e
fromError (Error e) = e
fromError _         = undefined

-- Please, do not refactor it.
stupidEval :: Expr -> [Prim Double] -> Except EvaluationError (Annotated [Prim Double] Double)
stupidEval (Val x)        st = Success (x :# st)
stupidEval (Op (Add a b)) st =
  let resAE = stupidEval a st
  in if (not . isSuccess) resAE then resAE else
       let resA :# stA = fromSuccess resAE
           resBE = stupidEval b stA
       in if (not . isSuccess) resBE then resBE else
            let resB :# stB = fromSuccess resBE
            in Success (evalExprInt' (+) a b :# (Add resA resB) : stB)
stupidEval (Op (Sub a b)) st =
  let resAE = stupidEval a st
  in if (not . isSuccess) resAE then resAE else
       let resA :# stA = fromSuccess resAE
           resBE = stupidEval b stA
       in if (not . isSuccess) resBE then resBE else
            let resB :# stB = fromSuccess resBE
            in Success (evalExprInt' (-) a b :# (Sub resA resB) : stB)
stupidEval (Op (Div a b)) st =
  let resAE = stupidEval a st
  in if (not . isSuccess) resAE then resAE else
       let resA :# stA = fromSuccess resAE
           resBE = stupidEval b stA
       in if (not . isSuccess) resBE then resBE else
            let resB :# stB = fromSuccess resBE
            in if resB == 0 then Error DivideByZero else Success (evalExprInt' (/) a b :# (Div resA resB) : stB)
stupidEval (Op (Mul a b)) st =
  let resAE = stupidEval a st
  in if (not . isSuccess) resAE then resAE else
       let resA :# stA = fromSuccess resAE
           resBE = stupidEval b stA
       in if (not . isSuccess) resBE then resBE else
            let resB :# stB = fromSuccess resBE
            in Success (evalExprInt' (*) a b :# (Mul resA resB) : stB)
stupidEval (Op (Abs a))   st =
  let resAE = stupidEval a st
  in if (not . isSuccess) resAE then resAE else
       let resA :# stA = fromSuccess resAE
       in Success (evalExprInt'' abs a :# (Abs resA) : stA)
stupidEval (Op (Sgn a))   st =
  let resAE = stupidEval a st
  in if (not . isSuccess) resAE then resAE else
       let resA :# stA = fromSuccess resAE
       in Success (evalExprInt'' signum a :# (Sgn resA) : stA)
